...
struct Api_AddDriverVehicle : Codable {
    let result : Res_AddDriverVehicle?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_AddDriverVehicle.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_AddDriverVehicle : Codable {
    let id : String?
