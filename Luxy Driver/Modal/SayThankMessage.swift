//
//  SayThankMessage.swift
//  Luxy Driver
//
//  Created by Techimmense Software Solutions on 17/01/25.
//

import Foundation

struct Api_ThankMessage : Codable {
    let result : [Res_ThankMessage]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_ThankMessage].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_ThankMessage : Codable {
    let id : String?
    let user_id : String?
