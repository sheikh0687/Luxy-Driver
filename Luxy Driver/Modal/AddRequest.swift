...
//
//  Created by Techimmense Software Solutions on 25/08/23.
//

import Foundation

struct ApiAddRequest : Codable {
    let result : ResAddRequest?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(ResAddRequest.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct ResAddRequest : Codable {
    let id : String?
    let user_id : String?
    let amount : String?
    let branch : String?
